...
def _post_var(base_url: str, var_name: str, value: str, timeout_s: float) -> None:
    base = base_url.rstrip("/")
    path_var = urllib.parse.quote(var_name, safe="")
    q_val = urllib.parse.quote(str(value), safe="")
    url = f"{base}/api/custom-variable/{path_var}/value?value={q_val}"
...
def push_sink_state(cfg: Dict[str, Any], sink_name: str, muted: bool | None = None, volume_percent: int | None = None) -> None:
    """
    Push sink state to Bitfocus Companion custom variables.
    Variable names are generated as <sinkKey><suffix>, e.g. browserVol/browserMute.
    """
    comp = cfg.get("companion", {}) if isinstance(cfg, dict) else {}
    if not isinstance(comp, dict):
        return
    ...
    if not base_url or not comp.get("enabled"):
        return
    ...
    if volume_percent is not None:
        _post_var(base_url, f"{key}{vol_suffix}", str(volume_percent), timeout_s)

    if muted is not None:
        _post_var(base_url, f"{key}{mute_suffix}", "1" if muted else "0", timeout_s)
